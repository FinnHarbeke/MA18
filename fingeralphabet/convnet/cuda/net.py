...
class FingeralphabetNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.criterion = nn.CrossEntropyLoss().to(device=self.cuda0)
    ...
    def train(self, dataloader, every_batch=2000, save_path=None):
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            X = sample_batched['image_tensor'].to(device=self.cuda0)
            y = sample_batched['target_ind'].to(device=self.cuda0)
            outputs = self(X)
