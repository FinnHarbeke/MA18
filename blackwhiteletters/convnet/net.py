import torch
import torch.nn as nn
import torch.nn.functional as F
from input import preprocess
import os

class Net(nn.Module):

    def __init__(self):
        super(Net, self).__init__()
        # 1 input image channel, 6 output channels, 5x5 square convolution
        # kernel
        self.conv1 = nn.Conv2d(1, 6, 5)
        self.conv2 = nn.Conv2d(6, 16, 5)
        # an affine operation: y = Wx + b
        self.fc1 = nn.Linear(16 * 5 * 5, 120)
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 26)

    def forward(self, x):
        # Max pooling over a (2, 2) window
        x = F.max_pool2d(F.relu(self.conv1(x)), (2, 2))
        # If the size is a square you can only specify a single number
        x = F.max_pool2d(F.relu(self.conv2(x)), 2)
        x = x.view(-1, self.num_flat_features(x))
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x

    def num_flat_features(self, x):
        size = x.size()[1:]  # all dimensions except the batch dimension
        num_features = 1
        for s in size:
            num_features *= s
        return num_features

    def train(self, X, y, times, every=2000, info=False, save_path=None):

        print(X.size()) if info else None

        for epoch in range(times):

            running_error = 0
            for i in range(len(X)):
                print('round', i) if info else None

                target = y[i]
                #print(target[0])
                print('target:', letter(target)) if info else None

                output = self(X[i].unsqueeze(0))[0]
                print('output:', letter(output)) if info else None

                error = nn.MSELoss()(output, target)
                print('error:', float(error)) if info else None

                self.backprop(error)

                running_error += error.item()
                if i % every == every-1:
                    print('epoch:', epoch + 1, ',' , i+1, ',', 'error:', running_error/every)
                    running_error = 0

                    if save_path != None:
                        torch.save(self.state_dict(), save_path)




    def backprop(self, error, learning_rate=0.01):
        self.zero_grad()
        error.backward(retain_graph=True)

        #update

        for f in self.parameters():
            f.data.sub_(f.grad.data * learning_rate)


def letter(arr):
    if arr.size()[-1] != 26:
        raise ValueError('array length gotta be 26!!')

    ind = 0
    for i in range(26):
        ind = i if arr[i] > arr[ind] else ind

    return 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[ind]
